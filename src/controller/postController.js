...
postController.post('/', async (req, res, next) => { // 추억 등록
    try {
        ...
        if (error.code === 422) {
            return res.status(422).json({ message: '동일한 제목의 추억이 이미 존재합니다.' });
        } else {
            return next(error);
        }
    }
...
postController.get('/', async (req, res, next) => { // 게시글 목록 조회
	try {
...
groupController.put('/:id', async (req, res, next) => {//게시글 수정
    try {
